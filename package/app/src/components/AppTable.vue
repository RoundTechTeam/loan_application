...
import { TableColumn, TableOption } from './type';
...
const rowsPerpage =
  props.option.pagination?.rowsPerPage === undefined
    ? 50
    : props.option.pagination?.rowsPerPage;
...
<template>
  <div class="relative-position app-table full-width">
    <q-table
      ...
      :selected-rows-label="(v) => (v ? `${v} of ${sortedData.length}` : '')"
    ...
    >
      <template #header="props">
        <q-tr :props="props">
          <q-th align="left" v-if="option.multiSelection && sortedData.length">
            <q-checkbox
